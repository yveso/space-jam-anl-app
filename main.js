...
function displayPlayerCards() {
  ...
  for (let [playerName, playerStats] of playerMap.entries()) {
    ...
    newPlayerPER.innerText =
      "PER: " + playerStats[currentQuarter].toPrecision(4);
  ...
  if (parentDiv.id == "playersOnBench") {
    ...
    if (playersOnCourt >= maxPlayersOnCourt) {
      ...
      document.getElementById("currentPER").innerText =
        "Current PER: " + quarterAvePER.toPrecision(4);
...
function updateCardsInGame() {
  ...
  // a specific quarter.
  for (let [playerName, playerStats] of playerMap.entries()) {
    document.getElementById(playerName + "_card").children[1].innerText =
      "PER: " + playerStats[currentQuarter].toPrecision(4);
  }
...
function endQuarter() {
  ...
  document.getElementById("quarter").innerText =
    "Choose Players for Q" + (currentQuarter + 1);
...
function endGame() {
  ...
  document.getElementById("averagePER").innerText +=
    quarterAvePER.toPrecision(4) + " = " + averagePER.toPrecision(4);
...
function startNextQuarter() {
  ...
  if (playersOnCourt != maxPlayersOnCourt) {
    alert(
      "Choose exactly " + maxPlayersOnCourt + " players to be on the court."
    );
    return;
  }
  ...
  document.getElementById("start").innerText =
    "Q" + (currentQuarter + 1) + " is in progress";
